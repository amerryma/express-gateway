...
class PluginInstaller {
  constructor () {
    ...
    this.PACKAGE_PREFIX = 'express-gateway-plugin-';
  }
  ...
  runNPMInstallation ({ packageSpecifier, cwd, env }) {
    return new Promise((resolve, reject) => {
      ...
      npmInstall.on('error', _ => {
        reject('Cannot install', this.argv.package);
      });
      ...
      npmInstall.stdout.on('end', () => {
        ...
        if (output.length < 4) {
          reject('Cannot parse npm output while installing plugin.');
          process.exit();
        }
  ...
  get pluginKey () {
    ...
    if (this.packageName.startsWith(this.PACKAGE_PREFIX)) {
      abbreviatedPackageName =
        this.packageName.substr(this.PACKAGE_PREFIX.length);
  ...
  updateConfigurationFiles ({ pluginOptions, enablePlugin,
    ...
    return new Promise(resolve => {
      if (!this.pluginManifest) {
        reject(new Error('Configuration files require a plugin manifest.' +
          ' Execute PluginInstaller#runNPMInstallation first.'))
        return;
      }
      ...
      if (enablePlugin) {
        ...
        if (name !== abbreviatedPackageName) {
